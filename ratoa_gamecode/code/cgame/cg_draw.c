...
//		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
	...
	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE )
		CG_DrawHudIcon ( HUD_HEALTHICON, qtrue,  cgs.media.healthIconBlue );
	else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED )
		CG_DrawHudIcon ( HUD_HEALTHICON, qtrue,  cgs.media.healthIconRed );
	else
		CG_DrawHudIcon ( HUD_HEALTHICON, qtrue,  cgs.media.healthIcon );
	...
	if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE )
		CG_DrawHudIcon ( HUD_ARMORICON, qtrue, cgs.media.armorIconBlue );
	else if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED )
		CG_DrawHudIcon ( HUD_ARMORICON, qtrue, cgs.media.armorIconRed );
	else
		CG_DrawHudIcon ( HUD_ARMORICON, qtrue, cgs.media.armorIcon );
}
...
void CG_DrawTimer ( void ) {
	...
	s = va ( "%s%i:%i%i", negative? "-" : "", mins, tens, seconds );  //FIXME: why not use %02i, is it slower?
...
static void CG_DrawDeathNotice ( void ) {
	...
	if ( ( cgs.consoleMsgTimes[ i ] != 0 ) && ( cg.time - cgs.consoleMsgTimes[ i ] < cg_deathNoticeTime.integer ) && hudelement.inuse ) {
...
void CG_Draw2D ( stereoFrame_t stereoFrame ) {
	...
	*/
	...
	if (CG_IsTeamGametype() && cg_drawTeamOverlay.integer) {
			CG_DrawTeamOverlay ( qtrue, qfalse );
		}
